...
namespace MappingFeed.Web.Services.SubscribedFeed;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (existingSubscription is not null)
        {
            ...
            if (startCursorSince is null &&
                ...
                return $"This channel is already subscribed to `{feedType.ToCommandValue()}` ({FeedEnumExtensions.BuildFilterSummary(feedType, existingSubscription.Rulesets, existingSubscription.EventTypes, existingSerializedGroupIds)}).";
        }
        ...
        return existingSubscription is null
            ? $"Subscribed this channel to `{feedType.ToCommandValue()}` ({FeedEnumExtensions.BuildFilterSummary(feedType, serializedRulesets, serializedEventTypes, serializedGroupIds)})."
            : $"Updated `{feedType.ToCommandValue()}` subscription ({FeedEnumExtensions.BuildFilterSummary(feedType, serializedRulesets, serializedEventTypes, serializedGroupIds)}).";
    }
