...
public static class OsuBeatmapsetEventsParser
{
    ...
    public static OsuBeatmapsetEventsPayload Parse(JsonObject root)
    {
        ...
        return new OsuBeatmapsetEventsPayload(
            events,
            users,
            reviewsMaxBlocksRaw is null ? null : (int)Math.Clamp((long)reviewsMaxBlocksRaw.Value, int.MinValue, int.MaxValue));
    }
    ...
    private static IReadOnlyList<OsuBeatmapsetEventsEvent> ParseEvents(JsonObject root)
    {
        ...
        foreach (var eventNode in Enumerable.OfType<JsonObject>((root.TryGetArray("events") ?? [])))
        {
    ...
    private static OsuBeatmapsetEventsEvent? ParseEvent(JsonObject eventNode)
    {
        ...
        if (!string.IsNullOrWhiteSpace(createdAtRaw) &&
            DateTimeOffset.TryParse((string?)createdAtRaw, out var parsedCreatedAt))
        {
    ...
    private static IReadOnlyList<OsuBeatmapsetEventsUser> ParseUsers(JsonObject root)
    {
        ...
        foreach (var userNode in Enumerable.OfType<JsonObject>((root.TryGetArray("users") ?? [])))
        {
